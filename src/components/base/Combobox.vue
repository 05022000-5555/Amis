<template>
  <div class="combobox">
    <input
      ...
      :class="{ 'm-input-error': checkError }"
      :title="[checkError ? 'Thông tin này không được phép để trống' : '']"
      v-model="textInput"
      @input="inputOnChange"
    ...
    >
      <table class="tblUnit">
        <thead>
          <tr>
            <th class="text-align-left" style="margin-left:20px">Mã đơn vị</th>
      ...
      <div class="update_organization flex gap-8">
        <div class="icon mi-16 mi-pen "></div>
        <p>Cập nhập cơ cấu tổ chức</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    hideListData() {
      ...
      // this.emitter.$emit("clickOutSide", this.textInput)
    },
    ...
    itemOnSelect(item, index) {
      const comboboxInput = document.querySelector(".combobox__input");
      const text = item[this.propText];
      const value = item[this.propValue];
      this.textInput = text; // Hiển thị text lên input.
      this.indexItemSelected = index;
      this.isShowListData = false;
      this.$emit("getValue", value, this.textInput);
      comboboxInput.classList.remove("m-input-error");
    },
